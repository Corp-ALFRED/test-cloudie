...
class Bill_Item_Custom_Fields {
	...
	public static function fields_form() {
		...
		$form_table .= '<thead><tr><th></th><th></th><th class="bill-table-title-tori">取引年月日</th><th>品目</th><th>数量</th><th>単位</th><th>単価</th><th>税抜/税込</th><th >消費税率</th><th>行の追加・削除</th></tr></thead>';
		...
		foreach ( $bill_items as $key => $bill_item ) {
			...
			foreach ( $tax_array as $tax_rate ) {
				...
				// 表示用消費税率
				$form_tax_rate = $tax_rate !== '0%' ? $tax_rate : __( '非課税', 'cloud-ie-system' );
				//オプションを表示
