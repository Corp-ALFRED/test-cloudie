...
/*
  納品書発行のボタン追加
/*-------------------------------------------*/
add_action( 'post_submitbox_start', 'bill_duplicate2' );
function bill_duplicate2() {
	global $post;
	$links = admin_url() . 'post-new.php?master_id=' . $post->ID;
	...
	<a href="<?php echo esc_url( $links ) . '&post_type=nohin&table_copy_type=all'; ?>" class="button button-default button-block">この内容で納品書を発行</a>
