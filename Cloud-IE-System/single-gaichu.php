...
<body <?php body_class(); ?>>
	<?php if (have_posts()) { ?>
		...
		?>
			<?php
			$doc_change = false;
			add_filter('project-pilot-doc-change', $doc_change);
			if (!$doc_change) {
				...
				} elseif (get_post_type() == 'gaichu') {
					get_template_part('template-parts/doc/frame-gaichu');
				}
			}
			do_action('project-pilot-doc-frame');
			?>
		<?php endwhile; ?>
	<?php } ?>
