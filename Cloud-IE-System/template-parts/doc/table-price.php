...
<style>
	table.table-bill thead tr th {
		background-color: <?php echo nl2br( esc_attr( $options['s-table-bg-color'] ) ); ?> !important;
	}
	table.table-bill.table-striped tbody tr:nth-of-type(odd) td {
		background-color: <?php echo nl2br( esc_attr( $options['s-table-bg-color'] ) ); ?>4D !important;
	}
	table.table-bill-total tbody tr th {
		background-color: <?php echo nl2br( esc_attr( $options['s-table-bg-color'] ) ); ?> !important;
	}
	table.table-bill-total tbody tr td {
		background-color: <?php echo nl2br( esc_attr( $options['s-table-bg-color'] ) ); ?>4D !important;
	}
...
<thead>
<tr>
<th class="text-center bill-cell-days">取引年月日</th>
...
$bill_item_sub_fields = array( 'tori','name', 'count', 'unit', 'price' );
$bill_total           = 0;
	...
	foreach ( $bill_items as $key => $bill_item ) {
		...
		if ( 
			...
			if ( is_numeric( $item_count ) && is_numeric( $item_price ) && is_numeric( $item_tax_rate_value ) ) :
				...
				$form_item_tax_rate   = $item_tax_rate !== '0%' ? $item_tax_rate : __( '非課税', 'cloud-ie-system' );
		        ...
		        if ($bill_items[$key]['tori']) {
                $bill_item_tori = $bill_items[$key]['tori'];
                $bill_item_tori = strtotime($bill_item_tori);
                $bill_item_tori = date("Y年m月d日", $bill_item_tori);
                } else {
                        $bill_item_tori = '　';
                    }
				?>
				<td class="tori-disp"><?php echo esc_html($bill_item_tori); ?></td>
...
<?php if ( ! empty( $tax_total ) && is_array( $tax_total ) ) : ?>
	<?php foreach ( $tax_total as $total ) : ?>
		...
		// 表示用消費税率
		$form_tax_rate = $total['rate'] !== '0%対象' ? $total['rate'] : __( '非課税', 'cloud-ie-system' );
		// 表示用消費税額
